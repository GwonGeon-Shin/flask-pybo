...
from pybo import db
from pybo.forms import UserCreateForm, UserLoginForm, UserPassNew
...
@bp.route('/newpass_ss/', methods=('GET', 'POST'))
@login_required
def newpass_ss():
    form = UserPassNew()
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        g.user.password = password1
        db.session.commit()
        flash('비밀번호를 변경 하였습니다.')



    # 해당 구문 비밀번호 변경 내용 적용하기

    return render_template('auth/new_pass.html', form=form)
