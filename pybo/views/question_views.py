    ...
    if request.method =='POST':
        ...
        for row in range(1, ws.max_row+1):
            ...
            try:
                ...
                test_bb_00 = test_bb[0]
                test_bb_01 = test_bb_00.our_ref
            except:
                pass

            if print_ques in test_bb_01:
                test_bb_00.pay_date=ws.cell(row,2).value
                test_bb_00.req_name=ws.cell(row,3).value
                test_bb_00.cus_name=ws.cell(row,4).value
                test_bb_00.date=ws.cell(row,5).value
                test_bb_00.number=ws.cell(row,6).value
                test_bb_00.q_date=ws.cell(row,7).value
                test_bb_00.q_d_date=ws.cell(row,8).value
                test_bb_00.d_d_date=ws.cell(row,9).value
                test_bb_00.d_d_num=ws.cell(row,10).value
                test_bb_00.p_name=ws.cell(row,11).value
                test_bb_00.charge_01=ws.cell(row,12).value
                test_bb_00.charge_02=ws.cell(row,13).value
                test_bb_00.p_charge=ws.cell(row,14).value
                db.session.commit()
            else:
                try:
